...
class ProtocolMixin(object):
    ...
    def stellar_sign_transaction(self, txEnvelope, address_n, networkPassphrase=None):
        ...
        for opIdx in range(0, parsed["num_operations"]):
            ...
            # Path Payment
            if op["type"] == 2:
                destination_asset = types.StellarAssetType(type=op["destination_asset"]["type"], code=op["destination_asset"]["code"], issuer=op["destination_asset"]["issuer"])
            ...
            # Manage Offer
            if op["type"] == 3:
                selling_asset = types.StellarAssetType(type=op["selling_asset"]["type"], code=op["selling_asset"]["code"], issuer=op["selling_asset"]["issuer"])
                buying_asset = types.StellarAssetType(type=op["buying_asset"]["type"], code=op["buying_asset"]["code"], issuer=op["buying_asset"]["issuer"])
